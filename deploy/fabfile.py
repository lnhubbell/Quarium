...
def _install_supervisor():
    ...
    # )
    sudo('mv Quarium/deploy/supervisor.conf /etc/supervisor/conf.d/Quarium.conf')
...
def _move_nginx_files():
    ...
    sudo('mv Quarium/deploy/simple_nginx_config /etc/nginx/sites-available/default')
...
def _mass_install():
    ...
    with settings(warn_only=True):
        sudo('pip install -r Quarium/requirements.txt')
